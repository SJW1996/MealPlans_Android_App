...
class AddProfileImage  : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mButtonUpload?.setOnClickListener(View.OnClickListener {
            if (mUploadTask != null && mUploadTask!!.isInProgress) {
                Toast.makeText(this@AddProfileImage, "Upload in progress", Toast.LENGTH_SHORT)
                    .show()
            } else {
                uploadFile()
            }
        ...
        if (mImageUri != null) {
            ...
            mUploadTask = fileReference.putFile(mImageUri!!)
                .addOnSuccessListener { taskSnapshot ->
                    ...
                    var uid = user!!.uid
                    mDatabaseRef?.child(uid)?.child("profileImage")?.setValue(url)
